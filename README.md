...
原始链接：http://rpc.yunfei.plus/yunfei-rpc/docs/

指数退避重试策略

我们可以实现一个指数退避重试策略。主要步骤如下:

定义最大重试次数 MAX_RETRY_TIMES 为 5 次。

在 doRetry() 方法中,使用 Stopwatch 来记录每次重试的耗时。
在每次重试时,先调用 callable.call() 执行远程调用。
如果出现异常,则进行重试处理:
    ...
    public RpcResponse doRetry(Callable<RpcResponse> callable) throws Exception {
        ...
        while (retryTimes < MAX_RETRY_TIMES) {
            try {
                ...
                stopwatch.reset();
                //程序运行持续时间elapsedtime，相当于修正退避时间，比如100ms后再次发送，但是程序要运行10ms，所以
                //只能sleep90ms
