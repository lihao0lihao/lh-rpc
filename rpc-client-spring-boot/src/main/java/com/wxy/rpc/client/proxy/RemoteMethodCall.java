...
import com.wxy.rpc.client.retry.RetryStrategy;
...
public class RemoteMethodCall {
    ...
    public static Object remoteCall(ServiceDiscovery discovery, RpcClient rpcClient, String serviceName,
        ...
        if (responseRpcMessage == null) {
            //超市重传机制
            Callable<RpcMessage> callable=()->{return rpcClient.sendRpcRequest(metadata);};
            try {
                responseRpcMessage = retryStrategy.doRetry(callable);
            } catch (Exception e) {
                e.printStackTrace();
            }
